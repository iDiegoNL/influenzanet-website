...
# get users 
users = dbQuery("SELECT id, last_login, date_joined from auth_user where is_active=true")

# get intake questionnaires
intake = survey_load_results("intake", c('timestamp'))
...
graph.open(my.path('inscrits'), width=600, height=400)
  ...
  if(Sys.Date() > as.Date('2012-01-01')) {
	  start.gpn = as.Date('2012-01-01')
