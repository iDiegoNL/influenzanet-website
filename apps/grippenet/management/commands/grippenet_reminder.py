...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        ...
        make_option('-e', '--exclude-final', action='store_true',  dest='exclude_final', help='Exclude participants with final questionnaire filled', default=False),
    )
    ...
    def handle(self, *args, **options):
        ...
        exclude_final = options.get('exclude_final')
        ...
        if exclude_final:
            r = self.get_respondents('pregnant')
            rr = set(respondents)
            rr.update(r)
            respondents = rr
