...
DROP FUNCTION IF EXISTS pollster_active_users_2012();
-- returns the global_id for active users in a given date
CREATE OR REPLACE FUNCTION pollster_active_users_2012 (
  date, -- $1 current day
	...
	SELECT global_id
	  FROM (
			...
			SELECT W.global_id,
				   ...
				   count(*) AS rate
			  FROM pollster_results_weekly_2012 W
...
DROP FUNCTION IF EXISTS pollster_ili_users_2012();
-- returns the global_id for active users with ILI onset on a given date
CREATE OR REPLACE FUNCTION pollster_ili_users_2012 (
  date, -- $1 current day
  min_survey int default 4 -- $2 min number of survey needed to be active
) RETURNS TABLE (
  global_id text
...
SELECT DISTINCT A.global_id
      FROM pollster_health_status_2012 S,
           pollster_results_weekly_2012 W,
           pollster_active_users_2012($1,$2) A
...
DROP FUNCTION IF EXISTS pollster_ili_incidence_2012();

CREATE FUNCTION pollster_ili_incidence_2012 (
  date, -- $1 first day
  date  -- $2 last day
) RETURNS TABLE (
  "day"         date,
...
SELECT 
  ...
  FROM (
    SELECT p_day,
          (SELECT count(*) FROM pollster_active_users_2012(p_day)) AS active,
          (SELECT count(*) FROM pollster_ili_users_2012(p_day)) AS ili
...
GRANT EXECUTE ON FUNCTION pollster_active_users_2012(date, integer) TO GROUP "DBA";
GRANT EXECUTE ON FUNCTION pollster_active_users_2012(date, integer) TO GROUP "Statistician";
GRANT EXECUTE ON FUNCTION pollster_ili_incidence_2012(date, date) TO GROUP "DBA";
GRANT EXECUTE ON FUNCTION pollster_ili_incidence_2012(date, date) TO GROUP "Statistician";
GRANT EXECUTE ON FUNCTION pollster_ili_users_2012(date, integer) TO GROUP "DBA";
GRANT EXECUTE ON FUNCTION pollster_ili_users_2012(date, integer) TO GROUP "Statistician";

-- TO RUN
-- select * from pollster_ili_incidence_2012('2012-11-15','2013-06-01')
