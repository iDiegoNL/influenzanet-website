...
AS  $$
        ...
        FROM (SELECT X.country, X.zip_code_key 
                FROM pollster_zip_codes X, (SELECT geometry FROM pollster_zip_codes Y WHERE Y.zip_code_key = $1) G
               WHERE ST_Touches(X.geometry, G.geometry)
              ) A 
...
CREATE OR REPLACE FUNCTION pollster_dashboard_badges_by_global_id(
  ...
  global_id     varchar(36),
  "user"       	integer,
  is_loyal_2012_2013 	boolean,
  is_loyal_2013_2014	boolean,
  is_novice	boolean,
  is_junior	boolean,
  is_senior	boolean,
  is_gold	boolean,
...
SELECT DISTINCT
                         A.global_id,
                         user_id AS "user",
                         sum(loyal_candidate_2012_2013) OVER (PARTITION BY A.global_id) > 0
                                AND (SELECT count(*)
                                FROM pollster_results_intake AS I
                               WHERE (timestamp <= '2013-04-30' AND I.global_id = A.global_id)) > 0
                         AS is_loyal_2012_2013,
                         sum(loyal_candidate_2013_2014) OVER (PARTITION BY A.global_id) > 0
                                AND (SELECT count(*)
                                FROM pollster_results_intake AS I
                               WHERE (timestamp > '2013-04-30' AND timestamp <= '2014-04-30' AND I.global_id = A.global_id)) > 0
                         AS is_loyal_2013_2014,
                         count(is_current_season) OVER (PARTITION BY A.global_id) >= 1 AS is_novice,
                         count(is_current_season) OVER (PARTITION BY A.global_id) >= 3 AS is_junior,
                         count(is_current_season) OVER (PARTITION BY A.global_id) >= 6 AS is_senior,
                         count(is_current_season) OVER (PARTITION BY A.global_id) >= 10 AS is_gold,
                         count(is_current_season) OVER (PARTITION BY A.global_id) >= 20 AS is_platinum,
...
SELECT DISTINCT HS.global_id,
                         user_id,
                   to_char(timestamp, 'YYYYWW') AS ts,
                   CASE WHEN timestamp > '2014-9-1' THEN true ELSE null END AS is_current_season,
                   CASE true WHEN (timestamp > '2012-04-30' AND timestamp <= '2013-04-30') THEN 1 ELSE 0 END as loyal_candidate_2012_2013,
                   CASE true WHEN (timestamp > '2013-04-30' AND timestamp <= '2014-04-30') THEN 1 ELSE 0 END as loyal_candidate_2013_2014,                   
                   CASE true WHEN (status ILIKE '%COLD%' AND timestamp > '2014-04-30') THEN 1 ELSE 0 END as cold,
                   CASE true WHEN (status = 'ILI' AND timestamp > '2014-04-30') THEN 1 ELSE 0 END as ili,
                   CASE true WHEN (status ILIKE '%GASTRO%' AND timestamp > '2014-04-30') THEN 1 ELSE 0 END as gastro
