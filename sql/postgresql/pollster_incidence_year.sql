...
DROP FUNCTION IF EXISTS pollster_active_users_{year}();
-- returns the global_id for active users in a given date
CREATE OR REPLACE FUNCTION pollster_active_users_{year} (
  date, -- $1 current day
...
DROP FUNCTION IF EXISTS pollster_ili_users_{year}();
-- returns the global_id for active users with ILI onset on a given date
CREATE OR REPLACE FUNCTION pollster_ili_users_{year} (
  date, -- $1 current day
  min_survey int default 4 -- $2 min number of survey needed to be active
) RETURNS TABLE (
  global_id text
...
SELECT DISTINCT A.global_id
      FROM pollster_health_status_{year} S,
           pollster_results_weekly_{year} W,
           pollster_active_users_{year}($1,$2) A
...
DROP FUNCTION IF EXISTS pollster_ili_incidence_{year}();

CREATE FUNCTION pollster_ili_incidence_{year} (
  date, -- $1 first day
  date  -- $2 last day
) RETURNS TABLE (
  "day"         date,
...
SELECT 
  ...
  FROM (
    SELECT p_day,
          (SELECT count(*) FROM pollster_active_users_{year}(p_day)) AS active,
          (SELECT count(*) FROM pollster_ili_users_{year}(p_day)) AS ili
...
GRANT EXECUTE ON FUNCTION pollster_active_users_{year}(date, integer) TO GROUP "DBA";
GRANT EXECUTE ON FUNCTION pollster_active_users_{year}(date, integer) TO GROUP "Statistician";
GRANT EXECUTE ON FUNCTION pollster_ili_incidence_{year}(date, date) TO GROUP "DBA";
GRANT EXECUTE ON FUNCTION pollster_ili_incidence_{year}(date, date) TO GROUP "Statistician";
GRANT EXECUTE ON FUNCTION pollster_ili_users_{year}(date, integer) TO GROUP "DBA";
GRANT EXECUTE ON FUNCTION pollster_ili_users_{year}(date, integer) TO GROUP "Statistician";
