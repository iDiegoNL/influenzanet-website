...
# options:
#
#   -b, --dbname: name of the local database to query (containing the epidb/pollster tables);
#                 default is "flusurvey"
#   -t, --tables: name of the tables to query (pollster/epidb)
#                 default is "pollster"
#   -c, --chart: create a motionchart HTML file (as opposed to a CSV table);
...
$fromstring .= " FROM $tables\_results_intake AS I, $tables\_health_status";
if ($definition ne "") {
    $fromstring .= "_$definition";
}
$fromstring .= " AS S, $tables\_results_weekly AS W";
