...
SELECT extract(year from age(to_timestamp(I."Q2",'YYYY-MM'))) AS a,
 ...
 WHERE S.pollster_results_weekly_id = W.id
   AND W.global_id = I.global_id AND extract(year from age(to_timestamp(I."Q2",'YYYY-MM'))) >= 0 AND C.postcode = upper(I."Q3")
       ) AS statuses
 GROUP BY country,agegroup ORDER BY country,agegroup;
