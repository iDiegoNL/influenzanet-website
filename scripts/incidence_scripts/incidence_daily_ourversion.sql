...
DROP FUNCTION IF EXISTS pollster_active_users_ourversion (date);

CREATE OR REPLACE FUNCTION pollster_active_users_ourversion (
  date -- $1 current day
) RETURNS TABLE (
  global_id text
)
...
DROP FUNCTION IF EXISTS pollster_ili_users_ourversion (date);

-- returns the global_id for active users with ILI onset on a given date
CREATE OR REPLACE FUNCTION pollster_ili_users_ourversion (
  date -- $1 current day
) RETURNS TABLE (
  global_id text
)
...
DROP FUNCTION IF EXISTS pollster_ili_incidence_ourversion (date, date);

-- returns the incidence ratio for a time period
CREATE OR REPLACE FUNCTION pollster_ili_incidence_ourversion (
  date, -- $1 first day
  date -- $2 last day
) RETURNS TABLE (
  "Day" date,
...
SELECT p_day AS "Day", active AS "Active", ili AS "ILI"
  FROM (
   SELECT p_day,
          ...
          (SELECT count(*) FROM pollster_ili_users_ourversion(p_day))::integer AS ili
