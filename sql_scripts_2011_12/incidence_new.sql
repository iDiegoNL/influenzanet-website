...
SELECT DISTINCT A.global_id
     ...
     WHERE S.pollster_results_weekly_id = W.id
       ...
       -- Ignore any symptoms which started before the 1st survey
       AND COALESCE(W."Q3_0_open", W.timestamp)::date > A.first
...
AS $body$
SELECT p_day AS "Day", CASE active WHEN 0 THEN 0 ELSE ili::float/active*1000.0 END AS "Incidence"
  FROM (
    SELECT p_day,
